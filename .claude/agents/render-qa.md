...
name: render-qa
description: >-
  Verifies LinuxStreamDeck key, startup, screen-saver, clean-exit display, and
  built-in game rendering offscreen without launching the GUI. Use after
  changes to device/renderer.py, device/startup_animation.py,
  device/screensaver.py, device/exit_display.py, games/*_render.py,
  games/render.py, games/rendering.py, core/icons.py, icon/game assets, or
  active-state logic, or whenever the user asks to verify how keys or
  full-deck displays look.

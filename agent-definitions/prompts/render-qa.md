...
- Minesweeper: lobby, hidden/revealed/numbered cells, Reveal/Flag modes, flags,
  mine/explosion/wrong-flag states and win/loss results. Results must preserve
  the final field, use the former mode key for Again and place Back somewhere
  other than the exploded cell.
- Tic-Tac-Toe: lobby, player X, the visible AI turn, AI O, winning lines,
  draw/player/AI results and both centered 3x3 and compact boards. Results must
  preserve and highlight the winning line with Again/Back outside it; compact
  boards need a subtle whole-board AI-wait tint, while larger boards use one
  status tile and subdued extras rather than repeating the same message.
- Colour Mastermind: lobby, cycling pegs, Submit/Reset, exact/colour-only clue
...
its one/two/three-mine difficulties; compact Tic-Tac-Toe must expose at least one
valid three-cell line; and Mastermind must retain usable clue and result controls
